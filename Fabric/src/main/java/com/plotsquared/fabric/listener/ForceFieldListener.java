...
public class ForceFieldListener {
    ...
    private static Set<PlotPlayer<?>> getNearbyPlayers(ServerPlayer player, Plot plot) {
        Set<PlotPlayer<?>> players = new HashSet<>();

        for (ServerPlayer nearPlayer :
                PlayerLookup.around(player.serverLevel(), player.position(), 5d).stream()
                .filter(entity -> entity instanceof ServerPlayer)
                .toList()
        ) {
            PlotPlayer<?> plotPlayer;
            if ((plotPlayer = FabricUtil.adapt(nearPlayer)) == null || !plot
    ...
    private static PlotPlayer<?> hasNearbyPermitted(ServerPlayer player, Plot plot) {
        for (ServerPlayer nearPlayer : PlayerLookup.around(player.serverLevel(), player.position(), 5d).stream()
                .filter(entity -> entity instanceof ServerPlayer)
                .toList()
        ) {
            PlotPlayer<?> plotPlayer;
    ...
    private static Vec3 calculateVelocity(PlotPlayer<?> player, PlotPlayer<?> e) {
    ...
    public static void handleForcefield(ServerPlayer player, PlotPlayer<?> plotPlayer, Plot plot) {
        if (plot.getFlag(ForcefieldFlag.class)) {
            ...
            if (plot.isAdded(uuid)) {
                ...
                if (!plotPlayer.hasPermission(Permission.PERMISSION_ADMIN_ENTRY_FORCEFIELD)) {
                    player.setDeltaMovement(calculateVelocity(oPlayer, plotPlayer));
                }
            }
        }
    }

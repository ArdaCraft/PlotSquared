...
public class FabricEntityUtil {
    ...
    public static boolean entityDamage(Entity damager, Entity victim, DamageSource cause) {
        Location dloc = FabricUtil.adapt(GlobalPos.of(damager.level().dimension(), damager.blockPosition()));
        Location vloc = FabricUtil.adapt(GlobalPos.of(victim.level().dimension(), victim.blockPosition()));
